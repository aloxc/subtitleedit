...
{
    public class EdiusMarkerList3Frames : SubtitleFormat
    {
        public override string Extension => ".csv";

        public override string Name => "Edius Marker List 3 Frames";
...
# No, Anchor, Position, Duration, Comment");
        ...
        public override void LoadSubtitle(Subtitle subtitle, List<string> lines, string fileName)
        {
            ...
            if (!text.Contains("# EDIUS Marker list") || !text.Contains("# Format Version 3"))
            {
                _errorCount++;
                return;
            }
            ...
            foreach (var line in lines)
            {
                ...
                {
                    if (parts.Count == 5 &&
                            ParseTimeCode(parts[2], out var startMs) &&
                            ParseTimeCode(parts[3], out var durationMs))
                    {
                        var p = new Paragraph(parts[4], startMs, startMs + durationMs);
        ...
        {
            var arr = tc.Split(':', '.');
            if (arr.Length == 4 && arr[3].Length == 2 &&
                ...
                int.TryParse(arr[3], NumberStyles.Any, CultureInfo.InvariantCulture, out var frames))
            {
                var timeCode = new TimeCode(hours, minute, seconds, FramesToMillisecondsMax999(frames));
